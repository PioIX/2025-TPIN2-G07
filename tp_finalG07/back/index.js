...
app.get("/traerUsuarios", async function (req, res) {
  try {
    let respuesta = await realizarQuery(
      `SELECT * FROM Usuarios WHERE username = "${req.query.username}"`
    );
...
app.get("/usuarios", async function (req, res) {
  ...
  if ((check.length = 0)) {
    ...
    res.send(
      await realizarQuery(
        `SELECT id_usuario FROM Usuarios WHERE username = '${req.body.username}'`
      )
    );
  }
});
...
app.post("/crearUsuario", async function (req, res) {
  try {
    ...
    if (check.length > 0) {
      ...
      let resTemp = await realizarQuery(
        `SELECT idUser FROM Usuarios WHERE nombre = "${req.body.nombre}"`
      );
      await realizarQuery(
        `UPDATE Usuarios SET fotoPerfil = "https://robohash.org/${resTemp[0].idUser}" WHERE idUser = ${resTemp[0].idUser}`
      );
      res.send(
        await realizarQuery(
          `SELECT * FROM Usuarios WHERE nombre = '${req.body.nombre}'`
...
app.post("/agregarASala", async function (req, res) {
  console.log(req.body);
  let check = await realizarQuery(
    `SELECT * FROM UsuariosPorSala WHERE idRoom = ${req.body.idRoom} AND idUser = ${req.body.idUser} `
  );
  if (check.length > 0) {
